...
**日付:** 2026-08-27 / **結果: 断念（20260827p で撤回）**

> **結論（先に読む）。この iPhone / iOS では実現できない。**
> `viewport-fit=cover` を付けると、ホーム画面アプリの窓は広がらず、
> **「画面の高さ − ステータスバー」のまま上へ移動する**。上で 47px 得て、下で 47px 失う。
> 下の 47px は **iOS 側の領域**で、ページからは色ひとつ塗れない（地色を紫にしても紫にならない）。
>
> **試してすべて駄目だったもの:** 3点セット（cover ＋ capable ＋ black-translucent）を揃える／
> アイコンを削除して入れ直す／`height=device-height`／`--app-h` で実寸に合わせる／
> `maximum-scale=1.0, user-scalable=no` を外す。最後のものは一度「効いた」ように見えたが、
> スクリーンショットの読み違いだった（実機では帯は消えていない）。
>
> **3回目を始める前にこの文書を読むこと。** 2026-08-18 と 2026-08-27 に失敗している。
...
「窓は 844 あって中の座標だけ 797 なのでは？（なら別の組み方で届く）」
→ **違う。** `html` の地色を紫にして確かめた結果、下の帯は紫にならなかった。
つまり帯はページのキャンバスの外＝iOS の領域。`body 797` / `map bottom 797` のまま。

## 判定でやってはいけないこと

- **スクリーンショットを目で読んで「届いた」と判断しない。** 一度これで誤判定し、
  「解決した」と報告してしまった。数十pxの帯は縮小画像では地図の縁と見分けがつかない。
- **`innerHeight` だけでも判断しない。** iOS はステータスバーぶん小さい値を返し続ける。
- 確実なのは **実機の目視**か、**画面に置いた色帯**を実機で見ること。

## 残したもの

`--safe-top` / `--safe-bottom` と、それを足す各所の計算は残してある。
cover が無い今はどこでも 0 なので何も変わらない。iOS が cover でも窓を縮めなくなったら、
meta を1行足すだけで全部が正しい位置に動く。確認は「cover を戻して、実機で下端に帯が出るか」。
