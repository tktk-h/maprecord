...
### スライド2「今年訪れた場所」のピン投入テンポ（加速カーブ）

- **全体時間 T ≈ 5秒で刺さりきる**（件数に依らずおおむね固定）。
- **最初の数本はゆっくり "ぽつ…ぽつ…"、時間が進むほど加速**（最後は連続でパパッと刺さる）。
- スケジュール式: N本のうち i 番目（0..N-1）の着地時刻 `t_i = T × (1 - (1 - i/(N-1))^k)`、**k ≈ 2〜2.5**（k が大きいほど序盤ゆっくり・終盤の加速が強い）。これで 1本ごとの間隔が「最初は広く → だんだん狭く」＝加速になり、N の大小に依らず同じ"間合い"に収まる。
- **数字カウントアップはピン着地と同期**（i 本目が刺さった瞬間に数字も i を表示）。最後の1本は軽くポンと強調して数字が確定。
- **件数がごく少ない年（例 N ≦ 5）**は T=5秒だと序盤の間隔が空きすぎて間延びするので、**T を短縮**（上限の目安 `T = min(5秒, N × 0.6秒)`）して詰める。
