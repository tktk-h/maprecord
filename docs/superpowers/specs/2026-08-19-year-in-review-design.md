...
- 今年訪れた場所: 数字を `count` まで **0からカウントアップ**しつつ、**SVG地図にピンが1記録ずつ次々に刺さる**（数字とピン投入を同期）→ 刺さり終わりに薄い線で軌跡をつなぐ。**主役スライド**。
- はじめての場所: 数字カウントアップ。
...
- **スライド構成（中身のあるものだけ自動表示）**:
  ...
  2. **今年訪れた場所**（常に表示 / `count>=1`）: 「今年訪れた場所」の数字を **おでかけ回数（`count`＝記録の数）** まで **0からカウントアップ**しながら、**SVGあしあと地図（§6）にピンが1記録につき1本ずつ次々に刺さっていく**演出。数字とピン投入を同期。全ピンが刺さり終わったら**薄い線で軌跡を軽くつなぐ**（旧・単体地図スライドの役割はここへ統合）。※ピンは1記録=1本なので同一場所は重なって刺さる。
