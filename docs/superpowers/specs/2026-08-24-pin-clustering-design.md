...
クラスタバッジをタップすると、そのクラスタの範囲にズーム&パンして中身のピンを展開する(標準的な地図アプリの挙動)。

## まとまる基準

ライブラリの既定アルゴリズムをそのまま使う。基準は「実際の距離(m)」ではなく**画面上のピクセル距離**(既定半径 約60px)。ズームアウトしているときは画面上60pxが実際の数百m〜kmに相当するため遠い地点同士もまとまり、ズームインすると数m〜数十mの近接ピンしかまとまらなくなる — 縮尺に応じて自動調整される。
...
**[js/app.js](../../../js/app.js)**: `cluster-toggle`ボタンのクリックでlocalStorage値を反転し、`App.records.render()`相当を呼んで再描画。

## エッジケース

- CDN読み込み失敗時: `window.markerClusterer`が存在しないため、exifrと同じガード(`if (window.markerClusterer)`)でクラスタなしの通常描画に自動フォールバックする
- クラスタ1件のみ: ライブラリが自動的に通常ピン表示にする
