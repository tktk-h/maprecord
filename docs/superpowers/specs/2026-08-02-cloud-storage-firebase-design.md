...
## 改訂 (2026-08-02): 方式B（Firestoreのみ・カード不要）で開始
Firebase Storage は Blaze プラン（￥5,000前払い／カード登録）が必要と判明したため、**当面はカード不要の方式Bで開始**する。写真は**圧縮して data URL（Base64）として Firestore に保存**する。将来大量になったら Storage（方式A）へ移行する。

- **写真の保存先**：Firebase Storage ではなく **Firestore**（記録ドキュメント内に `photos: [{ url: dataURL }]` として保存）。
- **表示側は URL 前提のまま**（`photo.url` を `img.src` に使う）。B では `url` が data URL、A では Storage のダウンロードURL。→ **表示コードは共通**で、移行が容易。
- **制約**：Firestore の1ドキュメント上限は1MB。写真は強めに圧縮（長辺~1280px / JPEG ~0.72、1枚 ~120〜180KB）し、1記録あたり合計が1MBを超えないようにする（超過時は保存エラーを表示）。
- **無料枠**：Firestore 保存 約1GB＝圧縮写真で約2,000〜2,500枚が目安。超えそうになったら方式Aへ。
- **初期設定から Storage 作成・Blaze アップグレードは不要**。Firestore と Auth のみ。
- **将来 A への移行**：写真の data URL を Storage にアップロードし、`photos` を `[{path,url}]` に貼り替える一度きりの移行処理を用意（別途）。

以下の本文は方式A前提で書かれた箇所が残るが、当面は上記Bを正とする（Storage・Storageルール・アップロード処理は方式A移行時に使う）。
